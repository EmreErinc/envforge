...
pub fn parse_project_config(
    ...
    match format {
        ...
        ConfigFormat::Yaml => serde_yaml::from_str(content).map_err(|e| ProjectError::ParseError {
            path: path.to_path_buf(),
            details: e.to_string(),
        }),
...
pub fn serialize_project_config(
    ...
    match format {
        ConfigFormat::Toml => {
            toml::to_string_pretty(config).map_err(|e| ProjectError::ParseError {
                path: PathBuf::new(),
                details: e.to_string(),
            })
        }
        ConfigFormat::Yaml => serde_yaml::to_string(config).map_err(|e| ProjectError::ParseError {
            path: PathBuf::new(),
            details: e.to_string(),
        }),
        ConfigFormat::Json => {
